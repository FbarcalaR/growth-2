...
- [x] 3.R.11 **Visual fidelity pass against `plans-tab.jsx`.** Reviewer flagged on PR #22 that the implementation didn't match the design at all. Walked the prototype thoroughly and rewrote the surface to match:
  ...
  - **Compact `<GoalCard>` summary in the list, drawer for editing.** Reviewer feedback after the first pass: keep the bottom-sheet drawer for goal management (preferred over inline expansion) but match the styling inside it to the prototype. Final shape: `<GoalCard>` is a compact summary (plant/seed icon + title + status chips + chevron + growth bar / banner) that opens `<GoalDetailSheet>` on tap. The drawer renders the design's full detail UI: large `<GoalIcon>` + bold title + chips at the top, "🌱 Plant now in garden" CTA when seed, growth bar / banner, Tasks + Routines editors, and full-width Edit goal / Delete goal buttons at the bottom (gated by `<ConfirmDialog>` for delete). Border colour on the card reflects state (seed-yellow / wilting / ill / critical / dead-grey / default).
