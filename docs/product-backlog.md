...
- [x] 3.R.11 **Visual fidelity pass against `plans-tab.jsx`.** Reviewer flagged on PR #22 that the implementation didn't match the design at all. Walked the prototype thoroughly and rewrote the surface to match:
  ...
  - **Inline-expandable `<GoalCard>`** (third and final shape after a couple of round-trips). The prototype renders goal management inline inside the card on `/garden`, so the card now toggles open/closed on header tap. Header is the plant-or-seed icon + title + status chips + chevron + growth bar / banner; expanded body holds the tasks + routines editors, an optional **Mark goal as complete** yellow CTA when every item is done (`useCompleteGoal` mutation), and a full-width **Edit goal** / **Delete goal** button row (delete still gated by `<ConfirmDialog>`). `<GoalDetailSheet>` was dropped entirely — there is no separate drawer route. Per-row icon affordances (edit / delete / graduate) are hidden by default and revealed via `group-hover` / `group-focus-within` so the row reads cleanly when the user is just scanning.
  ...
  - **Tests rewritten**: `goal-card.spec.tsx` drives the click-opens-drawer pattern + the seed/dead/blooming banners; `goal-editor.spec.tsx` walks the two-step flow + back button + edit-mode prefill; `garden/page.spec.tsx` walks New → Step 0 → Next → Step 1 → Add as seed. `goal-detail-sheet.spec.tsx` was deleted in the first pass; not restored because the drawer is exercised through the page test now.
  - **Deferred from the prototype** (called out so they don't get silently dropped):
    - Swipeable task / routine rows. The icons are hidden until hover or keyboard focus, but a real swipe-to-reveal gesture matching the prototype's `SwipeableItemRow` lands in the design-polish epic. Touch users without keyboard navigation currently see the icons after a `:focus-within` (e.g. tab into the row) — a stop-gap until the swipe lands.
