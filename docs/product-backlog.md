...
- [x] 3.R.11 **Visual fidelity pass against `plans-tab.jsx`.** Reviewer flagged on PR #22 that the implementation didn't match the design at all. Walked the prototype thoroughly and rewrote the surface to match:
  ...
  - **Per-row icon affordances** (edit / delete / graduate on tasks + routines) are hidden by default (`opacity-0`) and revealed via `group-hover` / `group-focus-within` so the row reads cleanly when the user is just scanning. Real swipe-to-reveal is documented as a deferral for the design-polish epic.
